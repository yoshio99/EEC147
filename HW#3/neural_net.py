...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    reg_term = 0.5 * reg * (np.sum(np.square(W1)) + np.sum(np.square(W2)))
    loss = loss + reg_term
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
    ...
    grad_h1 = np.dot(grad, W2)
    grads['W2'] = np.dot(grad.T, h1) + (reg * W2)
    grad_relu = g(h1) * grad_h1
    grads['b1'] = np.sum(grad_relu, axis=0)
    grads['W1'] = np.dot(grad_relu.T, X) + (reg * W1)
  ...
  def predict(self, X):
    ...
    for i in np.arange(len(scores)):
        max_idx = 0
        max_val = -np.inf
        for j in np.arange(len(scores[i])):
