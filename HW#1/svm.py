...
class SVM(object):
  ...
  def loss(self, X, y):
    ...
    # ================================================================ #
        z_j = 0
        correct_label_score = np.dot(self.W[y[i]].T,X[i])
        for j in np.arange(num_classes):
            if j != y[i]:
                class_score = np.dot(self.W[j].T,X[i])
                z_j = z_j + max(0, 1 + class_score - correct_label_score)
        loss += z_j
    loss = loss / num_train
  ...
  def loss_and_grad(self, X, y):
    ...
    # ================================================================ #
        ...
        for j in np.arange(num_classes):
            class_score = np.dot(self.W[j].T,X[i])
            score = 1 + class_score - correct_label_score
            if j != y[i]:
                z_j = z_j + max(0, score)
                if score > 0:
                    grad[j] += X[i]
  ...
  def fast_loss_and_grad(self, X, y):
    ...
    correct_scores = scores_mat[np.arange(scores_mat.shape[0]),y]
    z_js = np.maximum(0, 1 + scores_mat - np.matrix(correct_scores).T)
    z_js[np.arange(len(X)), y] = 0
    loss = np.sum(z_js, axis = 1)
    loss = np.sum(loss)
    loss = loss / len(X)
    # ================================================================ #
    # END YOUR CODE HERE
    ...
    # ================================================================ #
    z_js[z_js > 0] = 1
    z_js_summed = np.sum(z_js, axis=1)
    z_js[np.arange(len(X)),y] = -z_js_summed.T
    grad = np.dot(X.T, z_js)
    grad = grad.T / len(X)
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
